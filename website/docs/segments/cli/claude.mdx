...
| `.SessionName`          | `string`        | Custom name or AI-generated title; empty when neither exists                         |
...
| `.RateLimits`           | `RateLimits`    | Rate limit data for Claude.ai subscribers; nil when unavailable                      |
...
| `.PR`                   | `PR`            | Pull request or GitLab merge request information; nil when absent                    |
| `.Worktree`             | `Worktree`      | Claude Code worktree information; nil when absent                                    |
| `.FastMode`             | `bool`          | Whether fast mode is enabled; `false` when absent                                    |
...
| `.GitWorktree` | `string`   | Linked git worktree name; empty in the main working tree                     |
...
`.OutputStyle`, `.Effort`, `.Thinking`, `.Vim`, `.Agent`, `.PR`, `.Worktree`, and `.RateLimits` are `nil` when the
underlying data is absent. The same applies to `.Workspace.Repo`, `.ContextWindow.CurrentUsage`, and each rate limit
window. Guard direct field access with `{{if .Effort}}{{.Effort.Level}}{{end}}` to avoid template errors.
:::
...
| Name           | Type     | Description                                                  |
| -------------- | -------- | ------------------------------------------------------------ |
| `.Number`      | `string` | Review item number for the current branch; empty when absent |
| `.URL`         | `string` | Review item URL for the current branch; empty when absent    |
| `.ReviewState` | `string` | Review status; empty when unavailable                        |
| `.Kind`        | `string` | Review kind; `mr` for GitLab and empty for GitHub            |

For GitLab merge requests, `.ReviewState` uses different rules. It is `approved` when GitLab reports the MR as
mergeable. It is `draft` for a draft MR, and `pending` for other open states. GitLab MR data requires Claude Code
2.1.234 or later.
...
| Name                  | Type         | Description                                                  |
| --------------------- | ------------ | ------------------------------------------------------------ |
| `.TotalCostUSD`       | `float64`    | Estimated session cost in USD; resets to zero after `/clear` |
| `.TotalDurationMS`    | `string`     | Total session duration in milliseconds (formats as "Xm Ys")  |
| `.TotalAPIDurationMS` | `string`     | Time spent waiting for API responses (formats as "Xm Ys")    |
...
| Name                   | Type           | Description                                                                |
| ---------------------- | -------------- | -------------------------------------------------------------------------- |
| `.UsedPercentage`      | `int`          | Used context percentage; nil before usage data is available                |
| `.RemainingPercentage` | `int`          | Remaining context percentage; nil before usage data is available           |
| `.CurrentUsage`        | `CurrentUsage` | Last API call token counts; nil before the first call and after `/compact` |
| `.TotalInputTokens`    | `int`          | Input tokens in the current context, including cache reads and writes      |
| `.TotalOutputTokens`   | `int`          | Output tokens from the most recent API response                            |
...
| Name                        | Type  | Description                                     |
| --------------------------- | ----- | ----------------------------------------------- |
| `.InputTokens`              | `int` | Input tokens in the current context             |
| `.OutputTokens`             | `int` | Output tokens from the most recent API response |
| `.CacheCreationInputTokens` | `int` | Input tokens written to the prompt cache        |
| `.CacheReadInputTokens`     | `int` | Input tokens read from the prompt cache         |
...
| Name        | Type              | Description                                      |
| ----------- | ----------------- | ------------------------------------------------ |
| `.FiveHour` | `RateLimitWindow` | 5-hour rolling window; nil when unavailable      |
| `.SevenDay` | `RateLimitWindow` | 7-day rolling window; nil when unavailable       |

#### RateLimitWindow Properties

| Name              | Type      | Description                                            |
| ----------------- | --------- | ------------------------------------------------------ |
| `.UsedPercentage` | `float64` | Usage percentage from 0 to 100; nil when unavailable   |
| `.ResetsAt`       | `int64`   | Reset time as Unix epoch seconds; nil when unavailable |
